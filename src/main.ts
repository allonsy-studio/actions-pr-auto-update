...
type PRUpdateRun = Awaited<ReturnType<RestEndpointMethods["pulls"]["updateBranch"]>>;
...
export default async function run(core: typeof Core, github: typeof GitHub): Promise<void> {
	 ...
	 */
	async function fetchPullRequests(endpoint: RestEndpointMethods, limit = 100): Promise<Awaited<PRListPromise> | void> {
		try {
	...
	function filterPullRequests(prs: ReturnPullData): ReturnPullData | void {
		...
		try {
			...
			const includeDrafts: boolean | undefined = core.getInput("include_drafts") === "true" ? true : false;

			let strhold = core.getInput("include_labels");
			const allowLabels: string[] | undefined = typeof strhold !== "undefined" && strhold.length !== 0 ? strhold.split(",").map((i) => i.trim()) : undefined;

			strhold = core.getInput("exclude_labels");
			const denyLabels: string[] | undefined = typeof strhold !== "undefined" && strhold.length !== 0 ? strhold.split(",").map((i) => i.trim()) : undefined;

			if (typeof allowLabels === "undefined" && typeof denyLabels === "undefined" && includeDrafts) {
	...
	const limit: number | undefined = typeof strhold !== "undefined" && strhold.length !== 0 ? parseInt(strhold, 10) : undefined;
	...
	await Promise.all(
		...
		const passed = results.filter((r) => r!.result?.status === (200 as PRUpdateRun["status"]));
		const failed = results!.filter((r) => r!.result?.status !== (200 as PRUpdateRun["status"]));
